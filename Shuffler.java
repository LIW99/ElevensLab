...
public class Shuffler {
	...
	public static void perfectShuffle(int[] values) {
		...
		{
			half1[i] = values[i];
			//System.out.println("half1[" + i + "] = " + half1[i]);
		}
		//System.out.println("");
		...
		for (int i = 0; i < VALUE_COUNT - VALUE_COUNT/2; i++)
		{
			...
			//System.out.println("half2[" + i + "] = " + half2[i]);
		}
		//System.out.println();
		
		// Populate values with numbers from half1 & half2.  Each time through the loop
		// an item from half2 and an item from half1 are added into the values array.
		for (int i = 0; i < VALUE_COUNT/2; i++)
		{
			values[2*i] = half2[i];
	...
	public static void selectionShuffle(int[] values) {
		...
		for (int i = 0; i<values.length; i++)
		{
			...
			randomPosition = rand.nextInt(values.length);
